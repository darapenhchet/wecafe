		...
		<div class="content-page">
			...
			<div class="content">
				<div class="container">
					...
					<div class="row">
						<div class="col-sm-12">
							<h4 class="pull-left page-title">Update Expense Product</h4>
						</div>
					</div>
					<!-- Form-validation -->
					<div class="row">
										...
										<form class="cmxform form-horizontal tasi-form"
											...
											<div class="form-group ui-widget">
												<label for="productName" class="control-label col-lg-2">Product
													Name *</label>
												<div class="col-lg-10">
													<input class=" form-control" id="productName" name=""
														type="text"> <input type="text" class="hidebtn"
														id="prodID"> <input type="text" class="hidebtn"
														value="${EXPENSE_ID }" id="expid">
												</div>
											</div>
											...
											<div class="form-group ">
												...
												<div class="col-lg-10">
													<input class="form-control " id="supplierName"
														name="SupplierName" type="text"> <input
														type="text" class="hidebtn" id="suppId">
												</div>
											</div>
											<div class="form-group ">
												...
												<div class="col-md-12 col-sm-12 col-xs-12">
													<table id="datatable"
														...
														<tbody id="tbllistimport">
															<c:forEach items="${expensepro}" var="exppro"
																varStatus="theCount">
																<tr>
																	<%-- <td id="supplierId" style="display : none;">${supplier.supId}</td> --%>
																	<td>${theCount.count }</td>
																	<td id="proName">${exppro.exp_description }</td>
																	<td id="proqty">${exppro.exp_qty }</td>
																	<td id="prounitprice">${exppro.exp_unitprice }</td>
																	<td id="supname">${exppro.customer }</td>
																	<td id="remark">${exppro.remark }</td>
	...
	<script type="text/javascript">
            ...
            jQuery(document).ready(function($) { 
            	...
            	$(document).on("click","#btnedit",function(){
            		...
            		$("#productName").val($(this).parents("tr").children().eq(1).html());
            		$("#qty").val($(this).parents("tr").children().eq(2).html());
            		$("#UnitPrice").val($(this).parents("tr").children().eq(3).html());
            		$("#supplierName").val($(this).parents("tr").children().eq(4).html());
            		$("#remark").val($(this).parents("tr").children().eq(5).html()); 
            	});
                 	...
                 	var importDetail = [];
                 		$('#tbllistimport tr').each(function(){
                 			json ={
                 					...
                 					"proname"			:($(this).find("td").eq(1).html()),
     	 			...
     	 			$.ajax({
     	 				 url: "${pageContext.request.contextPath}/admin/saveupdate/" + $("#expid").val(), 
      				    type: 'POST',
     	 				datatype: 'JSON',
            	...
            	$(document).on("click","#updatebtn",function(){ 
            		...
            		_thisRow.children().eq(1).html($("#productName").val());
            		_thisRow.children().eq(2).html($("#qty").val());
            		_thisRow.children().eq(3).html($("#UnitPrice").val());
            		_thisRow.children().eq(4).html($("#supplierName").val()); 
            		_thisRow.children().eq(5).html($("#remark").val()); 
            		$("#updatebtn").html("Add");
            		$("#updatebtn").attr("id","addbtn");
            		clear();
            	});
